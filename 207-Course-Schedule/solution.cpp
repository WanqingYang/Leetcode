...
public:
    bool canFinish(int numCourses, vector<pair<int, int>>& prerequisites) {
        ...
        //if visited[i] is true,means from i to all nodes which i is their prerequisites, has no cycle
...
private:
    ...
    vector<bool> &visited) {
        ...
        return onpath[node] = false;// backtracking mark onpath[node] is false and return false
    }
};